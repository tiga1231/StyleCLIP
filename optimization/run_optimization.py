...
def find_edit(g_ema, args):
    """
    given textual description of the edit,
    edit the prescribed image accordingly

    Parameters
    ----------
    g_ema: preloaded styleGAN generator
    args: see get_parser()

    Returns
    -------
    result : a dict of results, including:
        final_result = [2, 3, 1024, 1024]:
        torch tensor of orignal and edited images
        latent_code_init: original latent code
        latent: edited latent code
    ...
    for i in pbar:
        ...
        if (args.save_intermediate_image_every > 0 and
            ...
            torchvision.utils.save_image(
                img_gen,
                f"{args.results_dir}/{str(i).zfill(5)}.jpg",
                normalize=True,
                range=(-1, 1),
            )
    ...
    return dict(
        ...
        latent=latent,
        latent_code_init=latent_code_init,
    )
...
def get_parser():
    ...
    parser.add_argument("--stylegan_size",
                        type=int,
                        default=1024,
                        help="StyleGAN resolution")
